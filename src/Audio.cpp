...
bool Audio::setPinout(uint8_t BCLK, uint8_t LRC, uint8_t DOUT, int8_t DIN) {
    ...
    #ifdef INTDAC  
        if(audio_info) audio_info("Internal DAC used");
        const esp_err_t result  = i2s_set_pin((i2s_port_t) m_i2s_num, NULL);
        return (result == ESP_OK);
    #else 
        if(audio_info) audio_info("External DAC used");
...
bool Audio::playSample(int16_t sample[2]) {
    ...
    uint32_t s32;
    #ifdef INTDAC
    int16_t l = Gain(sample[LEFTCHANNEL]) + 0x8000;
    int16_t r = Gain(sample[RIGHTCHANNEL]) + 0x8000;
    s32 = (r << 16) | (l & 0xffff);
    #else 
